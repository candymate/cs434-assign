...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    val nbStream = b.legalNeighbors.toStream
    nbStream.map(p => (p._1, p._2 :: history))
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial.isEmpty) Stream() // maybe assert this instead?
    else {
      val newNeighbors = newNeighborsOnly(neighborsWithHistory(initial.head._1, initial.head._2), explored)
      val newExplored = newNeighbors.map(_._1)
      initial.head #:: from(initial.tail #::: newNeighbors, explored ++ newExplored)
    }
  }
  ...
  lazy val solution: List[Move] = {
    try {
      ...
      case _: IndexOutOfBoundsException => List()
    }
  }
}
