...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
     ...
     */
    def always[T](value: T): Future[T] = Promise[T].success(value).future // Future(value)
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      // Future.sequence(fs)
      // reference: https://hamait.tistory.com/763 diagram at the end of the page
      fs.foldLeft(Promise[List[T]].success(List()).future) {
        (acc, ft) => for {
          head <- acc
          tail <- ft
        }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]
      fs.foreach(ft => p.completeWith(ft))
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]
      f.onComplete(t => p.success(cont(t)))
      p.future
    }
